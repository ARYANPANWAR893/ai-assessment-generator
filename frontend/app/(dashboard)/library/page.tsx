...
export default function LibraryArchiveView() {
  ...
  const filteredItems = items.filter(item => 
    (item.additionalInfo || "General Evaluation").toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    // 🛠️ FIX: Enforces a scroll layer inside the page element container bounds
    <div className="w-full h-full flex flex-col space-y-6 overflow-hidden font-sans">
      
      {/* Top Search Action Utility Block */}
      <div className="relative w-full max-w-md shrink-0">
        ...
        <input
          ...
          placeholder="Filter documents by info tag criteria..."
      ...
      {/* ✅ FIXED SCROLL CONTAINER: Re-enables independent native layout scrolling */}
      <div className="flex-1 overflow-y-auto pr-1 pb-6 space-y-4">
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-neutral-200 rounded-2xl bg-white">
            <p className="text-xs font-bold text-neutral-400 uppercase tracking-wider">No Valid Records Found</p>
            <p className="text-[11px] text-neutral-400 mt-1">Generate an assessment to populate this library view.</p>
          </div>
        ) : (
